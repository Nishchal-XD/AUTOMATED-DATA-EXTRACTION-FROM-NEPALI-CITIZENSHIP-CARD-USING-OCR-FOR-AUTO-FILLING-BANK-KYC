...
NEPAL_MUNICIPALITIES = {
    ...
    "sunsari": ["Itahari", "Dharan", "Inaruwa", "Duhabi", "Ramdhuni", "Barahachhetra", "Dewanganj", "Saptakoshi", "Gadhi", "Barju", "Bhokraha", "Harinagara"],
    ...
    # Bagmati Province
    "sindhuli": ["Kamalamai", "Dudhauli", "Sunkoshi", "Hariharpurgadhi", "Tinpatan", "Marin", "Golanjor", "Phikkal"],
    ...
    "baglung": ["Baglung", "Galkot", "Jaimuni", "Dhorpatan", "Bareng", "Kanthekhola", "Taman", "Tara Khola", "Nisikhola"],
...
def correct_municipality_name(ocr_text, district_name=None):
    ...
    # ========== ADD THIS FIX ==========
    # Fix for "hima" → "Himali"
    if cleaned == "hima" or cleaned == "हिमा":
        return "Himali"
    # =================================
...
def correct_district_name(ocr_text):
    ...
    district_variations = {
        ...
        "tanahu": "Tanahun",
        "tanahun": "Tanahun",
        "kailal": "Kailali",           # Add this
        "ramechha": "Ramechhap",       # Add this
        "ramecchap": "Ramechhap",      # Add this
        "ramechap": "Ramechhap",       # Add this
        "kavre": "Kavrepalanchok",     # Add common short forms
    ...
    try:
        ...
        district_names_lower = [d.lower() for d in NEPAL_DISTRICTS]
        
        # Find close matches with 70% similarity
        matches = difflib.get_close_matches(cleaned, district_names_lower, n=1, cutoff=0.7)
        
        if matches:
            # Find the original district name with correct capitalization
            for district in NEPAL_DISTRICTS:
                if district.lower() == matches[0]:
                    return district
...
def fix_nepali_name(text):
    ...
    text = re.sub(r'\bचौधरि\b', 'चौधरी', text)     
    text = re.sub(r'\bचौधरीय\b', 'चौधरी', text)    
    text = re.sub(r'\bडगीर\b', 'डगौरा', text)     
    text = re.sub(r'\bडगिर\b', 'डगौरा', text)    
    text = re.sub(r'\bडगीरी\b', 'डगौरा', text)    
    text = re.sub(r'\bडगौर\b', 'डगौरा', text)     
    text = re.sub(r'\bदगौरा\b', 'डगौरा', text)     
    text = re.sub(r'\bडोल्म\b', 'डोल्मा', text)
    text = re.sub(r'\bकामा\b', 'कामी', text)
    text = re.sub(r'कामा\s+ग्राडदेन\s+तामाङ', 'कामी ग्राडदेन तामाङ', text)
    text = re.sub(r'\bहिरादेविरेग्म\b', 'हिरादेवि रेग्मी', text)
    text = re.sub(r'\bहिरादेवीरेग्म\b', 'हिरादेवि रेग्मी', text)
    text = re.sub(r'\bहिरादेविरेग्मी\b', 'हिरादेवि रेग्मी', text)
    text = re.sub(r'\bहिरादेवि\b', 'हिरादेवि', text)
    text = re.sub(r'\bरेग्म\b', 'रेग्मी', text)
    text = re.sub(r'\bरेग्मी\b', 'रेग्मी', text)
    text = re.sub(r'(हिरादेवि)(रेग्मी)', r'\1 \2', text)
    text = re.sub(r'\bजत्रुखर\b', 'जत्रु खंग', text)
    text = re.sub(r'\bजत्रुखर्\b', 'जत्रु खंग', text)
    text = re.sub(r'\bलेकेन्ध्र\b', 'लोकेन्द्र', text)
    text = re.sub(r'\bलेकेन्द्र\b', 'लोकेन्द्र', text)
    text = re.sub(r'\bनन्दै\b', 'नन्दे', text)
    text = re.sub(r'\bनन्दे\b', 'नन्दे', text)
    text = re.sub(r'\bखत्व\b', 'खत्वे', text)
    text = re.sub(r'\bखत्व्\b', 'खत्वे', text)
    text = re.sub(r'\bसौर्बपिव\b', 'सौर्य पिया', text)
    text = re.sub(r'\bसौर्बपिय\b', 'सौर्य पिया', text)
    text = re.sub(r'\bसौर्बपिया\b', 'सौर्य पिया', text)
    text = re.sub(r'\bसौर्बपीय\b', 'सौर्य पिया', text)
    text = re.sub(r'\bपिय\b', 'पिया', text)
    text = re.sub(r'\bपिय्\b', 'पिया', text)
    text = re.sub(r'\bपीय\b', 'पिया', text)
    text = re.sub(r'\bहकको\s+पर\s+पुजाश्रेष्ट\b', 'पुजा श्रेष्ठ', text)
    text = re.sub(r'\bहकको पर पुजाश्रेष्ट\b', 'पुजा श्रेष्ठ', text)
    text = re.sub(r'\bहककोपरपुजाश्रेष्ट\b', 'पुजा श्रेष्ठ', text)
    text = re.sub(r'\bपुजाश्रेष्ट\b', 'पुजा श्रेष्ठ', text)
    text = re.sub(r'\bदार्गबहादुर\s+जोग\b', 'दुर्ग बहादुर जोगी', text)
    text = re.sub(r'\bदार्गबहादुर\b', 'दुर्ग बहादुर', text)
    text = re.sub(r'\bकाकाको\s+घरः\s+जाल\s+कुमारी\s+जोग\b', 'जाल कुमारी जोगी', text)
    text = re.sub(r'\bकुवर\b', 'कुंवर', text)
    text = re.sub(r'\bबन्नुक्रो\s+राम\s+अनुग्रह\s+राय\s+यादव\b', 'राम अनुग्रह राय यादव', text)
    text = re.sub(r'\bफण्डेय\b', 'पाण्डेय', text)
    text = re.sub(r'\bफण्डेय्\b', 'पाण्डेय', text)
    text = re.sub(r'\bफण्डे\b', 'पाण्डेय', text)
    text = re.sub(r'\bदिनाभट्ट\b', 'दिना भट्ट', text)
    text = re.sub(r'\bदिनाभट्ट्\b', 'दिना भट्ट', text)
    text = re.sub(r'\bजिबनाधमभट्ट\b', 'जिब नाथ भट्ट', text)
    text = re.sub(r'\bजिबनाधमभट्ट्\b', 'जिब नाथ भट्ट', text)
    text = re.sub(r'\bकोपिलाभट्ट\b', 'कोपिला भट्ट', text)
    text = re.sub(r'\bकोपिलाभट्ट्\b', 'कोपिला भट्ट', text)
    text = re.sub(r'\bपहर\b', 'पहरी', text)
    text = re.sub(r'\bराजपाण्डे\b', 'राज पाण्डे', text)
    text = re.sub(r'\bथयानेदार\b', 'थानेदार', text)
    text = re.sub(r'सीतारानीचौधरी', 'सीता रानी चौधरी', text)
    text = re.sub(r'सीतारानीचौधरि', 'सीता रानी चौधरी', text)
    text = re.sub(r'सीतारानी चौधरी', 'सीता रानी चौधरी', text)
    text = re.sub(r'सीता रानीचौधरी', 'सीता रानी चौधरी', text)
    text = re.sub(r'\bसुर्खाल\b', 'सुर्खाली', text)
    text = re.sub(r'\bरुरु\b', 'सरु', text)
    text = re.sub(r'\bरुरू\b', 'सरु', text)
    text = re.sub(r'\bरुरु\s*', 'सरु ', text)
    text = re.sub(r'\bईंदलेंसाह\b', 'इंदल साह', text)
    text = re.sub(r'\bईंदलें\s*साह\b', 'इंदल साह', text)
    text = re.sub(r'\bखत्र\b', 'खत्री', text)
    text = re.sub(r'\bख्त्र\b', 'खत्री', text)
    text = re.sub(r'\bख्जर\b', 'खत्री', text)
    text = re.sub(r'\bख्ज्र\b', 'खत्री', text)
    text = re.sub(r'\bनाग्न\s+थयरः\s+तक्ष्मौ\s+खत्र\b', 'लक्ष्मी खत्री', text)
    text = re.sub(r'\bनाग्न थयरः तक्ष्मौ खत्र\b', 'लक्ष्मी खत्री', text)
    text = re.sub(r'\bतक्ष्मौ\s+खत्र\b', 'लक्ष्मी खत्री', text)
    text = re.sub(r'\bठकुर\b', 'ठकुरी', text)
    text = re.sub(r'\bक्कर्ण\b', 'कर्ण', text)
    text = re.sub(r'\bचन्द्र\b', 'चन्द', text)
    text = re.sub(r'\bतेल\b', 'तेली', text)
    text = re.sub(r'\bतिर्षमाया\s+महर्जन\b', 'तिर्थमाया महर्जन', text)
    text = re.sub(r'\bतिर्षमाया\b', 'तिर्थमाया', text)  

    text = re.sub(r'^हकको पर\s+', '', text)
    text = re.sub(r'\bहकको\s*पर\s*', '', text)
    text = re.sub(r'\bकाकाको घरः\s+', '', text)
    text = re.sub(r'^नरम घरे\s+', '', text)        
    text = re.sub(r'\s+नरम घरे\s+', ' ', text)    
    text = re.sub(r'\bनरम\b', '', text)           
    text = re.sub(r'\bघरे\b', '', text) 
    text = re.sub(r'\bबन्नुक्रो\s+', '', text)
    text = re.sub(r'\bबाब्रकी परः\s+', '', text)
    text = re.sub(r'^बाब्रकी\s+परः\s+', '', text)
    text = re.sub(r'^आ्यार्को\s+नामुज्ञरर\s+', '', text)
    text = re.sub(r'^बर्बुको\s+', '', text)
    text = re.sub(r'\bजैँगमाको\s+', '', text)
    text = re.sub(r'\bजैँगमाको\b', '', text)
    text = re.sub(r'^जैँगमाको\s+', '', text)


    # ---- First-name character confusion ----
    # ज → श confusion at word-start (e.g. जुसिल → शुसिल, जुशिल → शुशिल)
    text = re.sub(r'^जु(सि|शि)', lambda m: 'शु' + m.group(1), text)
    text = re.sub(r'^जु(सी|शी)', lambda m: 'शु' + m.group(1), text)
    # Fix trailing त→ल in names like शुसित→शुसिल
    text = re.sub(r'(शुसि)त\b', r'\1ल', text)
    text = re.sub(r'(शुशि)त\b', r'\1ल', text)
    # ज → अ confusion at word-start (e.g. जमृता → अमृता)
    ...
    # ल → त / त → ल confusion at end of first names
    text = re.sub(r'(सुशी)त\b', r'\1ल', text)
    text = re.sub(r'(रोशा)त\b', r'\1ल', text)
    # ब → व confusion (bi- names that should start with वि-)
    ...
    # व → ब confusion at word-start (e.g. वादुराम → बाबुराम)
    # Tesseract reads initial 'ब' as 'व' in some fonts
    text = re.sub(r'^वादुराम\b', 'बाबुराम', text)    # वादुराम → बाबुराम
    text = re.sub(r'^वाबुराम\b', 'बाबुराम', text)    # वाबुराम → बाबुराम
    text = re.sub(r'^वाबु', 'बाबु', text)             # वाबु* → बाबु*
    text = re.sub(r'^वालु', 'बालु', text)             # वालु* → बालु*
    ...
    # व- prefix names that should be ब-
    text = re.sub(r'\bवाबुराम\b', 'बाबुराम', text)   # mid-string too
    ...
    _surnames = r'(कार्की|खड्गी|भित्रकोटी|पुडासैनी|चन्द|शाह|थापा|श्रेष्ठ|मगर|गुरुङ|तामाङ|गौचन)'
...
def post_process_extracted_data(data):
    ...
    for item in data.get('back', []):
        if item['class_id'] == 5 and item.get('text'):
            ...
            item['text'] = re.sub(r'^HE\s+', '', item['text'], flags=re.IGNORECASE)
            item['text'] = re.sub(r'\bHE\s+', '', item['text'], flags=re.IGNORECASE)
            item['text'] = re.sub(r'\bFEERIYA\b', 'RIYA', item['text'], flags=re.IGNORECASE)
            item['text'] = re.sub(r'DE', '', item['text'], flags=re.IGNORECASE)
            item['text'] = re.sub(r'\s+', ' ', item['text']).strip()
            item['text'] = re.sub(r'\bOUN\s+', '', item['text'], flags=re.IGNORECASE)
            item['text'] = re.sub(r'^OUN\s+', '', item['text'], flags=re.IGNORECASE)
    ...
    for item in data.get('back', []):
        if item['class_id'] == 5 and item.get('text'):  # english name
            ...
            item['text'] = re.sub(r'(?i)^Municipality\s+', '', item['text'])
            item['text'] = re.sub(r'(?i)\s+Municipality$', '', item['text'])    
            item['text'] = re.sub(r'(?i)\bMunicipality\b', '', item['text'])
            item['text'] = re.sub(r'\bBaiuwatar\b', 'Baluwatar', item['text'], flags=re.IGNORECASE)
